...
\begin{align}
    ...
    \vec\nabla \times \vec B - \frac{\partial}{\partial t}\vec E = \vec j.
\end{align}
$P$ and $T$ transformation for $\rho$ and $\vec j$ is
\begin{align}
    P :&\qquad \rho(t,\vec x) \to \rho(t,-\vec x), \qquad
    \vec j(t,\vec x) \to -\vec j(t,-\vec x), \\
    T :&\qquad \rho(t,\vec x) \to \rho(-t,\vec x), \qquad
    \vec j(t,\vec x) \to -\vec j(-t,\vec x).
\end{align}
Thus, $\vec E$ and $\vec B$ behaves as
\begin{align}
    P :&\qquad \vec E(t,\vec x) \to -\vec E(t,-\vec x), \qquad
    \vec B(t,\vec x) \to \vec B(t,-\vec x), \\
    T :&\qquad \vec E(t,\vec x) \to \vec E(-t,\vec x), \qquad
    \vec B(t,\vec x) \to -\vec B(-t,\vec x).
\end{align}
Thus, under $P$, $\vec E$ is odd and $\vec B$ is even. Under $T$, $\vec E$ is even and $\vec B$ is odd.\footnote{
Note that the scalar potential $\phi$ and vector potential $\vec A$ forms a four-vector $A^\mu = (\phi, \vec A)$. $\vec E$ and $\vec B$ are written in terms of $A^\mu$ as
\begin{align}
    ...
    = (-F^{23}, -F^{31}, -F^{12}),
\end{align}
where $F_{\mu\nu} = \partial_\mu A_\nu - \partial_\nu A_\mu$. From Maxwell equation $\partial_\mu F^{\mu\nu} = j^\nu$, we can observe that $A_\mu$ and $j_\mu$ should behave in the same way under $P$ and $T$ transformation, i.e., $A_0 \to A_0$ and $A_i \to -A_i$.}
...
As we will see in the next section, in non-abelian gauge theories, there is a non-perturbative contribution to make $\theta$ term relevant.
...
In the massless quark limit, the QCD Lagrangian is invariant under $U(3)_L \times U(3)_R$ chiral symmetry as
\begin{align}
...
\begin{align}
    U(3)_L \times U(3)_R \simeq SU(3)_L \times SU(3)_R \times U(1)_V \times U(1)_A. \label{eq:chiral symmetry}
\end{align}
...
\begin{align}
    U(3)_V = SU(3)_V \times U(1)_V. \label{eq:unbroken symmetry}
\end{align}
Comparing eqs.~\eqref{eq:chiral symmetry} and \eqref{eq:unbroken symmetry}, we can see that the number of broken generators is 9. Thus, we expect 9 massless NG bosons in the massless quark limit. According to the Nambu--Goldstone theorem, there should be 9 massless NG bosons corresponding to these broken generators.
Indeed, $\pi^0$, $\pi^+$, $\pi^-$, $K^+$, $K^-$, $K^0$, $\bar K^0$, and $\eta$ mesons are understood as NG bosons corresponding to the 8 broken generators of $SU(3)_L \times SU(3)_R \to SU(3)_V$. How about the remaining one, i.e., $U(1)_A$ symmetry?
...
\end{align}
This is the Dashen-Nuyts equation.
...
\begin{align}
    d_n
    \sim \frac{e}{16\pi^2} \frac{m_* \bar\theta c_+}{f_\pi} \frac{g_A m_N}{f_\pi} \frac{1}{m_N} \log\frac{\Lambda}{m_\pi}
    \sim e\bar\theta \times \frac{c_+ g_A m_*}{(4\pi f_\pi)^2} \log\frac{m_N}{m_\pi}
    \sim \bar\theta \times 10^{-16} ~e~{\rm cm},
\end{align}
where $\Lambda \sim 1~{\rm GeV}$.
